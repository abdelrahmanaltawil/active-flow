...
def discretize(L: float, N: int) -> tuple[list[np.ndarray]]:
    '''
    Placeholder
    '''
...
def solve(w_k: np.ndarray, ITERATIONS: int, tau: float, time_step: Callable, velocity: Callable, cfl_controller: Callable, energy: Callable) -> tuple[list]:
    ...
    for iteration in range(ITERATIONS+1):
        ...
        if iteration % 100 == 0:
            ...
            print(
                "iteration = "+"{0:07d}".format(iteration) +"\t" + "tau = "+"{:.4f}".format(tau)+ "\t" + 
                "E(k=1) = "+"{:.20s}".format("{:0.20f}".format(E_k_1)) +"\t"+ "U_max = "+"{:.20s}".format("{:0.20f}".format(max_u))
                )
    ...
    print('Simulation time = %s sec' % str(round(time.monotonic()-t0, 3))) 
