...
if __name__ == "__main__":
    ...
    parameters["postprocessing"]["snapshots_locations"] = list(map(int, reference_run["parameters/snapshots_locations"].fetch().replace('[', '').replace(']', '').split(",")))
    ...
    for symbol in ["all_extrema", "minima", "maxima"]:
        ...
        figure = postprocessing.plot_radial_profile_snapshots(
            k_modes= np.arange(1, np.max(re.register["operators"]["k_vectors"][:,:,0][0])),
            radial_profile_snapshots= re.register["snapshots_radial_profile"],
